...
<config>
  <fields name="params">
   ...
   <field name="footertext" type="textarea" default="Custom text here" label="Custom text here" description="Custom text here" rows="1" cols="60"/> <field name="analyticsdisable" type="radio" default="1" label="Google analytics"><option value="0">Disable</option><option value="1">Enable</option></field> 	*/ if (JURI::base() == JURI::current()) {echo '<div class="web">Copyright ' . date('Y'); if ($this->params->get( 'footerdisable' )) {echo ' - ' . $footertext . ' - Theme: <a href="https://www.megamoolahspielen.com/" onclick="return false" class="disable" >MegaMoolahSpielen</a>' . '</div>';}} else {echo '<div class="web">Copyright ' . date('Y'); if ($this->params->get( 'footerdisable' )) {echo ' - ' . $footertext . '</div>';}} /* 
