...
function photoEditor(url)
{
    ...
    function init()
    {
        ...
        $g('.aspect-ratio-select input[type="hidden"]').val('original').prev().val(galleryLanguage['ORIGINAL']);
        ...
        $g('#apply-save-copy').off('click').on('click', function(event){
            ...
            if (this.classList.contains('active-button')) {
                ...
                XHR.open("POST", JUri+'administrator/index.php?option=com_bagallery&task=uploader.checkFileExists', true);
                XHR.send(data);
            }
        });
    ...
    function saveImage()
    {
        ...
        if (obj.image.indexOf('data:') === 0) {
            ...
            var data = JSON.stringify(obj),
                XHR = new XMLHttpRequest(),
                str = galleryLanguage['SAVING']+'<img src="'+JUri+'components/com_bagallery/assets/images/reload.svg"></img>';
            ...
            setTimeout(function(){
                XHR.onreadystatechange = function(e){
                    if (XHR.readyState == 4) {
                        ...
                        showNotice(galleryLanguage['SUCCESS_UPLOAD']);
                        window.frames['upload-target'].location.href = window.frames['upload-target'].location.href;
                        setTimeout(function(){
                            $g('#photo-editor-dialog').modal('hide');
                        }, 3000);
                    }
                };
                XHR.open("POST", JUri+'administrator/index.php?option=com_bagallery&task=uploader.savePhotoEditorImage', true);
                XHR.send(data);
            }, 400);
        }
    }
...
app.photoEditor = function(){
    ...
    setTimeout(function(){
        if (itemDelete.url[0] == '/') {
            itemDelete.url = itemDelete.url.substr(1);
        }
        photoEditor(JUri+itemDelete.url);
    }, 600);
}
